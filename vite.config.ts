...
export default defineConfig(({ command, isPreview }) => ({
  ...
  plugins: [
    ...
    ...(command === "build" || isPreview
      ? [
          nitro({
            // Vercel stays the default target. `NITRO_PRESET=render-com`
            // (set in render.yaml) swaps in the Node server build instead —
            // Nitro's own env var is ignored once `preset` is passed inline.
